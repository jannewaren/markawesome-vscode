...
- **`<wa-icon>` Web Awesome 3.10.0 attributes** — the editor now reflects the widened icon vocabulary both engines accept (for `:::wa-icon …` **and** callout icon overrides):
  - **Component reference** (`components.json`): the **Icon** entry gains a `canvas` array (`fixed`/`auto`/`square`/`roomy`), the `semibold` variant, the nine new animations (`flip-360`, `spin-snap`, `spin-snap-4`, `spin-snap-8`, `buzz`, `float`, `jello`, `swing`, `wag`), and the full current WA family vocabulary (`mosaic`, `pixel`, `vellum`, `slab-duo`, `slab-press-duo`, `chisel`, `etch`, `graphite`, `jelly`, `jelly-duo`, `jelly-fill`, `notdog`, `notdog-duo`, `slab`, `slab-press`, `thumbprint`, `utility`, `utility-duo`, `utility-fill`, `whiteboard`); its `altSyntax` becomes `:::wa-icon name family? variant? animation? canvas?` and the doc prose covers `canvas`/`semibold`. The **Callout** entry's shared `families`/`animations` arrays are synced and its prose notes `semibold`/`canvas`.
  - **Hover** now renders an **Icon Canvas** line from the new `canvas` array.
  - **IntelliSense**: typing `:::wa-icon <name> ` (and inside a callout) now suggests the new families, `semibold`, the nine new animations, and a new `canvas` (`fixed`/`auto`/`square`/`roomy`) completion block.
  - **Snippets**: `wa-icon-animation` and `wa-callout-animation` choice placeholders include the nine new animations, and a new `wa-icon-canvas` snippet inserts a `canvas` sizing box.
...
- **Popover & tooltip aligned placements + `skidding`, and per-tab `disabled`** — tracking the engines' expanded `<wa-popover>`/`<wa-tooltip>`/`<wa-tab>` surface:
  - **Component reference** (`components.json`): Popover and Tooltip `placements` now list all twelve Web Awesome values (the four primary plus the eight aligned variants), `skidding:N` is added to their parameters, and the syntax/example/documentation are refreshed. The Tab Group entry documents the per-tab `disabled` flag.
  - **IntelliSense**: typing `&&&` (popover) and `(((` (tooltip) now offers the eight aligned placements and a `skidding:` token (alongside the existing `distance:`). A new completion on the `+++ ` tab item header line offers `disabled`.
  - **Snippets**: `wa-popover-skidding` and `wa-tooltip-skidding` (aligned placement + `skidding:`), `wa-tabs-disabled` (a tab group with a disabled tab), and the existing popover/tooltip placement snippets now list all twelve placements.
